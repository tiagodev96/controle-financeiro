...
test.describe('Lançar despesa (sheet)', () => {
  ...
  test('E3 — household sem categorias mostra empty state com CTA', async ({ page, context }) => {
    ...
    await expect(page.getByRole('button', { name: /^lançar$/i })).toHaveCount(0);
  });

  test('E4 — valor zero é rejeitado com mensagem inline e mantém o sheet aberto', async ({ page, context }) => {
    await loginAsTestUser(context);
    ...
    await expect(page).toHaveURL(/\/\?sheet=lancar/);
